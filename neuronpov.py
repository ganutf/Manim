...
class NeuronPOV(Scene):
    def construct(self):
        # Create a neuron
        neuron = Circle(radius=0.5, color=BLUE, fill_opacity=1, z_index=1)
        self.play(Create(neuron))
        self.wait()

        # Incoming connections
        num_incoming = 10
        incoming_lines = VGroup(*[Line(start=np.random.uniform(-5, 5, size=3), end=neuron.get_center(), stroke_width=2, color=YELLOW) for _ in range(num_incoming)])
        self.play(Create(incoming_lines), run_time=2)
        self.wait()

        # Incoming signals
        def activate_line(line):
            signal = Dot(color=RED).scale(0.5).move_to(line.get_start())
            return Succession(
                Create(signal),
                signal.animate.move_to(line.get_end()),
                FadeOut(signal)
            )
        ...
        self.play(neuron_activation, run_time=2)
        self.wait()

        # Outgoing connections
        num_outgoing = 5
        outgoing_lines = VGroup(*[Line(start=neuron.get_center(), end=np.random.uniform(-5, 5, size=3), stroke_width=2, color=GREEN) for _ in range(num_outgoing)])
